...
//========================================================================================================
// BEG DMI CHANGES
EPTBLandF = function(upItemStyle, downItemStyle, upMouseOverStyle, downMouseOverStyle, disabledStyle) {
...
//------------------------------------------------------------
EPToolBarItem.prototype.create = function(enabled) {
	...
	if (enabled) {
		...
		EP.applyStyle(this.elem, this.landf.disabledStyle);
	}
...
EPRTA.prototype.create = function(rtaDoc,rtaWin,rtaIFrame,initialHTML,spellCheckInProgress) {
	...
	for (tbItem in 	this.toolbarItems) {
		// BEG DMI CHANGES
		this.toolbarItems[tbItem].create(this.enabled);
		// END DMI CHANGES
	}
};
    ...
    for (var item = messageElement.firstChild; item; item = item.nextSibling) {
        ...
        for (var i = 0; i < landfs.length; i++) {
    		...
    		if (tag == "button") {
    	    		...
    	    		// BEG DMI CHANGES
    	    		var disabledStyle = landfItem.getAttribute("disabledStyle");
    				buttonLandF = new EPTBLandF(upItemStyle,downItemStyle,upMouseOverStyle, downMouseOverStyle, disabledStyle); 
    	    		// END DMI CHANGES
    		}
    		if (tag == "select") {
        ...
        // BEG DMI
        // General settings (currently only enabled)
        var settings = item.getElementsByTagName("setting");
        var setItem = null;
        for (var i = 0; i < settings.length; i++) {
        	setItem = settings[i];
        	key = setItem.getAttribute("key");
        	val = setItem.getAttribute("val");
        	if (key == "enabled") {
        		rta.enabled = val == "true" ? true : false;
			}
		}
		// END DMI
